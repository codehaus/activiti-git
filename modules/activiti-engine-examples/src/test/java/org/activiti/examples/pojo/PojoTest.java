  ...
  public void testDecision() {
    ...
    ObjectProcessDefinition processDefinition = ProcessDefinitionBuilder.createProcessDefinition().createActivity("start").initial().behavior(new Automatic())
            .transition("checkCredit").endActivity().createActivity("checkCredit").behavior(new Decision()).transition("takeToGolf",
                    new ExpressionCondition(scriptingEngines, "${creditRating=='Aaa-'}", ScriptingEngines.DEFAULT_EXPRESSION_LANGUAGE)).transition("askDaughterOut",
                    new ExpressionCondition(scriptingEngines, "${creditRating=='AAA+'}", ScriptingEngines.DEFAULT_EXPRESSION_LANGUAGE)).transition("ignore").endActivity()
